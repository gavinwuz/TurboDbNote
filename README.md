...
Title-bar actions and the left navigation rail are icon-only with tooltips; content buttons and settings categories use icons plus text. Tabs show an icon, title and close icon. The editor menu and tab context menu include Close all tabs; context menus also offer Close, Close others, Close right and Close left. The right-side AI Chat, Properties and Schema tabs are fixed, non-closable and retain their order. Closing keeps notebook and chat entities in memory; Help → Restore auxiliary tabs reopens the notebook and auxiliary tabs. Settings temporarily collapses auxiliary regions and restores their visibility on close.
...
Independent language packs are bundled in `crates/desktop/assets/locales/`. Put compatible JSON packs in `locales/` beside `settings.db`; restart to add or override a language, then select it in General. Packs contain `id`, `name` and a `strings` map; missing translations fall back to the source text. Theme packages use GPUI Component's ThemeSet JSON format in the adjacent `themes/` directory and appear under Additional themes.
